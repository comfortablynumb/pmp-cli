...
mod tests {
    ...
    /// Helper to set up a basic template pack
    fn setup_template_pack(fs: &MockFileSystem) {
  ...
  inputs:
    app_name:
      ...
      description: "Application name""#;
        ...
        fs.write(&template_dir.join("src/main.tf.hbs"), "# Template").unwrap();
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_create_new_infrastructure_basic() {
        ...
        input.add_response(MockResponse::Text("My Infrastructure".to_string())); // name
        input.add_response(MockResponse::Text("Test infrastructure".to_string())); // description
        input.add_response(MockResponse::MultiSelect(vec!["pmp.io/v1/TestResource".to_string()])); // resource kinds
        // Environment setup
        input.add_response(MockResponse::Text("dev".to_string())); // env key
        input.add_response(MockResponse::Text("Development".to_string())); // env name
        input.add_response(MockResponse::Text("Development environment".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // add another environment

        let ctx = create_test_context(Arc::clone(&fs), input);

        // Run init command
        let result = InitCommand::execute(&ctx, None, None, None);
        ...
        let current_dir = std::env::current_dir().unwrap();
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        assert!(fs.has_file(&infrastructure_file), "Infrastructure file should be created");

        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("name: My Infrastructure"), "Should contain infrastructure name");
        assert!(content.contains("description: Test infrastructure"), "Should contain description");
        assert!(content.contains("kind: TestResource"), "Should contain resource kind");
        assert!(content.contains("dev:"), "Should contain dev environment");
        assert!(content.contains("name: Development"), "Should contain environment name");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_name_from_cli_arg() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);

        // Set up mock user input (no name prompt expected)
        let input = MockUserInput::new();
        input.add_response(MockResponse::Text("CLI description".to_string())); // description
        input.add_response(MockResponse::MultiSelect(vec!["pmp.io/v1/TestResource".to_string()])); // resource kinds
        input.add_response(MockResponse::Text("dev".to_string())); // env key
        input.add_response(MockResponse::Text("Development".to_string())); // env name
        input.add_response(MockResponse::Text("".to_string())); // env description (optional)
        input.add_response(MockResponse::Confirm(false)); // add another environment

        let ctx = create_test_context(Arc::clone(&fs), input);

        // Run init command with CLI name arg
        let result = InitCommand::execute(&ctx, Some("CLI Infrastructure"), None, None);
        ...
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("name: CLI Infrastructure"), "Should use CLI-provided name");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_environment_validation_lowercase() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);
        ...
        // Set up mock user input
        let input = MockUserInput::new();
        input.add_response(MockResponse::Text("Test Infra".to_string())); // name
        input.add_response(MockResponse::Text("".to_string())); // description (optional)
        input.add_response(MockResponse::MultiSelect(vec!["pmp.io/v1/TestResource".to_string()])); // resource kinds
        input.add_response(MockResponse::Text("PRODUCTION".to_string())); // Invalid - uppercase
        input.add_response(MockResponse::Text("production".to_string())); // Valid - lowercase
        input.add_response(MockResponse::Text("Production".to_string())); // env name
        input.add_response(MockResponse::Text("".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // add another
        ...
        assert!(result.is_ok(), "Init should succeed after retry: {:?}", result);

        let current_dir = std::env::current_dir().unwrap();
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("production:"), "Should contain lowercase environment key");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_creates_projects_directory() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);
        ...
        // Set up mock user input
        let input = MockUserInput::new();
        input.add_response(MockResponse::Text("Test".to_string())); // name
        input.add_response(MockResponse::Text("".to_string())); // description
        input.add_response(MockResponse::MultiSelect(vec!["pmp.io/v1/TestResource".to_string()])); // resource kinds
        input.add_response(MockResponse::Text("dev".to_string())); // env key
        input.add_response(MockResponse::Text("Dev".to_string())); // env name
        input.add_response(MockResponse::Text("".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // add another
        ...
        let current_dir = std::env::current_dir().unwrap();
        let projects_dir = current_dir.join("projects");
        assert!(fs.exists(&projects_dir), "Projects directory should be created");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_multiple_environments() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);
        ...
        // Second environment
        input.add_response(MockResponse::Text("prod".to_string())); // env key
        input.add_response(MockResponse::Text("Production".to_string())); // env name
        input.add_response(MockResponse::Text("Prod env".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // add another
        ...
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("dev:"), "Should contain dev environment");
        assert!(content.contains("prod:"), "Should contain prod environment");
        assert!(content.contains("name: Development"), "Should contain dev name");
        assert!(content.contains("name: Production"), "Should contain prod name");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_environment_duplicate_detection() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);
        ...
        input.add_response(MockResponse::Confirm(true)); // add another
        // Duplicate key
        input.add_response(MockResponse::Text("dev".to_string())); // Duplicate!
        input.add_response(MockResponse::Text("staging".to_string())); // Retry with different key
        input.add_response(MockResponse::Text("Staging".to_string())); // env name
        input.add_response(MockResponse::Text("".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // done
        ...
        assert!(result.is_ok(), "Init should succeed after duplicate retry: {:?}", result);

        let current_dir = std::env::current_dir().unwrap();
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("dev:"), "Should contain dev environment");
        assert!(content.contains("staging:"), "Should contain staging environment");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_environment_key_with_hyphen() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());
        setup_template_pack(&fs);
        ...
        input.add_response(MockResponse::Text("".to_string())); // description
        input.add_response(MockResponse::MultiSelect(vec!["pmp.io/v1/TestResource".to_string()])); // resource kinds
        input.add_response(MockResponse::Text("dev-test".to_string())); // env key with hyphen (valid)
        input.add_response(MockResponse::Text("Dev Test".to_string())); // env name
        input.add_response(MockResponse::Text("".to_string())); // env description
        input.add_response(MockResponse::Confirm(false)); // done
        ...
        assert!(result.is_ok(), "Init should accept hyphens in environment keys: {:?}", result);

        let current_dir = std::env::current_dir().unwrap();
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("dev-test:"), "Should contain environment key with hyphen");
    }

    #[test]
    #[ignore] // TODO: MockFileSystem doesn't support directory discovery needed for template discovery
    fn test_init_multiple_resource_kinds() {
        // Set up mock filesystem
        let fs = Arc::new(MockFileSystem::new());

        // Set up multiple template packs with different resource kinds
        let current_dir = std::env::current_dir().unwrap();

        // First template pack
        let pack1_path = current_dir.join(".pmp/template-packs/pack1");
        fs.write(&pack1_path.join(".pmp.template-pack.yaml"), r#"apiVersion: pmp.io/v1
kind: TemplatePack
metadata:
  name: pack1
spec: {}"#).unwrap();

        let template1_dir = pack1_path.join("templates/template1");
        fs.write(&template1_dir.join(".pmp.template.yaml"), r#"apiVersion: pmp.io/v1
kind: Template
metadata:
  name: template1
spec:
  ...
  executor: opentofu
  inputs: {}"#).unwrap();
        fs.write(&template1_dir.join("src/main.tf.hbs"), "# Template 1").unwrap();

        // Second template pack
        let pack2_path = current_dir.join(".pmp/template-packs/pack2");
        fs.write(&pack2_path.join(".pmp.template-pack.yaml"), r#"apiVersion: pmp.io/v1
kind: TemplatePack
metadata:
  name: pack2
spec: {}"#).unwrap();

        let template2_dir = pack2_path.join("templates/template2");
        fs.write(&template2_dir.join(".pmp.template.yaml"), r#"apiVersion: pmp.io/v1
kind: Template
metadata:
  name: template2
spec:
  ...
  executor: opentofu
  inputs: {}"#).unwrap();
        fs.write(&template2_dir.join("src/main.tf.hbs"), "# Template 2").unwrap();

        // Set up mock user input
        let input = MockUserInput::new();
        input.add_response(MockResponse::Text("Test".to_string())); // name
        input.add_response(MockResponse::Text("".to_string())); // description
        input.add_response(MockResponse::MultiSelect(vec![
            "pmp.io/v1/KubernetesWorkload".to_string(),
            "pmp.io/v1/Database".to_string()
        ...
        let infrastructure_file = current_dir.join(".pmp.infrastructure.yaml");
        let content = fs.get_file_contents(&infrastructure_file).unwrap();
        assert!(content.contains("kind: KubernetesWorkload"), "Should contain KubernetesWorkload");
        assert!(content.contains("kind: Database"), "Should contain Database");
    }
}
