...
impl InfrastructureCommand {
    ...
    pub fn execute_init(
        ...
        let selected_template = if all_infra_templates.len() == 1 {
            ...
            let selected_option = ctx.input.select(
                "Select infrastructure template:",
        ...
        if generate_ci {
            ...
            let ci_provider = ctx
                .input
                .select("Select CI provider:", ci_providers, None)?;
    ...
    fn collect_executor_config(ctx: &Context) -> Result<ExecutorCollectionConfig> {
        ...
        let backend_type = ctx
            .input
            .select("Backend type:", backend_types.clone(), None)?;
